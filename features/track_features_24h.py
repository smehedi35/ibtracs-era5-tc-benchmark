...
def _compute_motion_and_targets(df: pd.DataFrame, id_col: str, time_col: str,
    ...
    """
    Compute motion features and targets for specified horizon.
    Assumes 6h cadence: horizon_hours ∈ {6, 12, 24} → shifts of {1, 2, 4}.
    """
    if horizon_hours not in [6, 12, 24]:
        raise ValueError(f"Unsupported horizon: {horizon_hours}h. Must be 6, 12, or 24.")
    
    steps = horizon_hours // 6  # 6h→1, 12h→2, 24h→4
    df = df.sort_values([id_col, time_col]).reset_index(drop=True)

    # previous displacement (t-horizon_hours → t)
    ...
    speed_prev_kmh = (np.array(prev_dist) / float(horizon_hours))  # km/h
    ...
    # next (target) displacement (t → t+horizon_hours)
...
def build_features(config_path: str):
    ...
    print(f"[DONE] {horizon}h feature build complete.")
