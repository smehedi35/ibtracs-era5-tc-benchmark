# scripts/track_forecast_baselines_6h.py - Enhanced baselines with climatology
...
REQUIRED = [
    ...
    "lat", "lon", "lat_next6", "lon_next6",
    "dx_6h", "dy_6h",  # Updated from dx_prev_km/dy_prev_km
    "u10", "v10"
]

KM_PER_MS_OVER_6H = 21.6
...
def run(args=None):
    ...
    df = df.dropna(subset=["lat_next6", "lon_next6"]).copy()
    
    out_frames = []
    
    # ========== Baseline 1: Persistence ==========
    dx_pers = df["dx_6h"].to_numpy(dtype=float)
    dy_pers = df["dy_6h"].to_numpy(dtype=float)
    latp, lonp = _predict_latlon_from_dxdy(df, dx_pers, dy_pers)
    err = great_circle_error_km(df["lat_next6"], df["lon_next6"], latp, lonp)
    
    base1 = df[["storm_id", "iso_time", "split", "lat", "lon", "lat_next6", "lon_next6"]].copy()
    base1["lat_pred_next6"] = latp
    base1["lon_pred_next6"] = lonp
    base1["error_km"] = err
    base1["method"] = "persistence"
    out_frames.append(base1)
    
    # ========== Baseline 2: Wind10 advection ==========
    dx_w10 = df["u10"].to_numpy(dtype=float) * KM_PER_MS_OVER_6H
    dy_w10 = df["v10"].to_numpy(dtype=float) * KM_PER_MS_OVER_6H
    latp2, lonp2 = _predict_latlon_from_dxdy(df, dx_w10, dy_w10)
    err2 = great_circle_error_km(df["lat_next6"], df["lon_next6"], latp2, lonp2)
    
    base2 = df[["storm_id", "iso_time", "split", "lat", "lon", "lat_next6", "lon_next6"]].copy()
    base2["lat_pred_next6"] = latp2
    base2["lon_pred_next6"] = lonp2
    base2["error_km"] = err2
    base2["method"] = "wind10_advection"
    out_frames.append(base2)
    
    # ========== Baseline 3: Climatology (basin-specific mean motion) ==========
    # Compute mean dx/dy per basin from training data
    train = df[df["split"] == "train"].copy()
    basin = df["storm_id"].str[:2].values
    train["basin"] = train["storm_id"].str[:2]
    
    clim_motion = train.groupby("basin")[["dx_6h", "dy_6h"]].mean().to_dict()
    dx_clim = np.array([clim_motion["dx_6h"].get(b, 0) for b in basin])
    dy_clim = np.array([clim_motion["dy_6h"].get(b, 0) for b in basin])
    
    latp3, lonp3 = _predict_latlon_from_dxdy(df, dx_clim, dy_clim)
    err3 = great_circle_error_km(df["lat_next6"], df["lon_next6"], latp3, lonp3)
    
    base3 = df[["storm_id", "iso_time", "split", "lat", "lon", "lat_next6", "lon_next6"]].copy()
    base3["lat_pred_next6"] = latp3
    base3["lon_pred_next6"] = lonp3
    base3["error_km"] = err3
    base3["method"] = "climatology"
    out_frames.append(base3)
    
    # ========== Baseline 4: Weighted ensemble (persistence 0.7 + wind10 0.3) ==========
    dx_ens = 0.7 * dx_pers + 0.3 * dx_w10
    dy_ens = 0.7 * dy_pers + 0.3 * dy_w10
    latp4, lonp4 = _predict_latlon_from_dxdy(df, dx_ens, dy_ens)
    err4 = great_circle_error_km(df["lat_next6"], df["lon_next6"], latp4, lonp4)
    
    base4 = df[["storm_id", "iso_time", "split", "lat", "lon", "lat_next6", "lon_next6"]].copy()
    base4["lat_pred_next6"] = latp4
    base4["lon_pred_next6"] = lonp4
    base4["error_km"] = err4
    ...
    pred_path = out_root / "predictions" / "baselines_6h.parquet"
    ...
    met_path = out_root / "metrics" / "baselines_6h.csv"
    ...
    for _, row in test_metrics.iterrows():
        print(f"{row['method']:25s}  {row['mean_km']:6.2f} km  "
              f"(median: {row['median_km']:5.2f} km, p95: {row['p95_km']:5.2f} km)")
    print("="*70 + "\n")
