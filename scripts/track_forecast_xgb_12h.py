# scripts/track_forecast_xgb_12h.py - Advanced 12h track forecasting model
...
# Feature list matching build_track_features_12h.py (62 features)
DEFAULT_FEATURES = [
    # Position & intensity
    ...
    # Advanced
    "storm_age_h", "wind_chg_6h", "wind_chg_12h", "wind_chg_24h",
    "motion_accel", "sst_gradient", "dist_to_land_km",
    "intensity_x_speed", "sst_x_wind", "mslp_deficit", "coriolis_param",
...
def train(args=None):
    ...
    must = DEFAULT_FEATURES + ["dx_km", "dy_km", "split", "lat", "lon", 
                               "lat_next12", "lon_next12", "sample_weight"]
    ...
    # Hyperparams tuned for 12h horizon (62 features)
    common = dict(
        n_estimators=5000,
        learning_rate=0.012,         # slower for 12h (more complex)
        max_depth=12,                # deeper (was 10 for 6h)
        num_leaves=95,               # more (was 63)
        subsample=0.75,              # lower for regularization
        colsample_bytree=0.7,        # 62 features → need more dropout
        reg_alpha=0.03,              # less L1 (was 0.05)
        reg_lambda=0.4,              # more L2 (was 0.3)
        min_child_samples=30,        # higher (was 25)
    ...
    m_dx.fit(
        ...
        eval_metric="l2",
        callbacks=[
            lgb.early_stopping(stopping_rounds=100, verbose=False),  # more patience for 12h
    ...
    joblib.dump(m_dx, out_root / "models" / "lgbm_dx_12h.joblib")
    joblib.dump(m_dy, out_root / "models" / "lgbm_dy_12h.joblib")
    ...
    feat_imp.to_csv(out_root / "models" / "feature_importance_12h.csv", index=False)
    ...
    def _predict_split(dfin: pd.DataFrame, split_name: str) -> pd.DataFrame:
        if dfin.empty:
            return dfin.assign(lat_pred_next12=np.nan, lon_pred_next12=np.nan, error_km=np.nan)
        ...
        for lt, lo, dx, dy in zip(dfin["lat"].values, dfin["lon"].values, dx_hat, dy_hat):
            ...
            latp.append(la)
            lonp.append(lo2)
        err = great_circle_error_km(dfin["lat_next12"], dfin["lon_next12"], latp, lonp)
        out = dfin[["storm_id", "iso_time", "lat", "lon", "lat_next12", "lon_next12", "split"]].copy()
        out["lat_pred_next12"] = np.asarray(latp)
        out["lon_pred_next12"] = np.asarray(lonp)
        out["error_km"] = err
    ...
    pred_path = out_root / "predictions" / "predictions_12h.parquet"
