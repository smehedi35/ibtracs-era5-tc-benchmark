# scripts/build_track_features_24h.py - Advanced 24h track forecasting features
...
REQUIRED = [
    ...
    "sst_c", "t2m_c", "msl_hpa",
    "u10", "v10", "wind10", "tp",
    "lat_next24", "lon_next24",
    "has_next24", "split", "sample_weight",
]

def compute_motion_features(df: pd.DataFrame, lookback: int = 4) -> Tuple[np.ndarray, ...]:
    """
    Motion from t-6h*lookback to t.
    For 24h: lookback=4 means t-24h to t (4 steps of 6h).
    """
...
def compute_intensity_change(df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    """6h, 12h, 24h, 48h intensity change (kt)."""
    ...
    wind_48h = df.groupby("storm_id")["wmo_wind"].diff(8)
    return (wind_6h.fillna(0).values, wind_12h.fillna(0).values, 
            wind_24h.fillna(0).values, wind_48h.fillna(0).values)

def compute_motion_acceleration(df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
    """
    Change in translation speed (acceleration) at 12h and 24h scales.
    Critical for 24h: steering pattern changes.
    """
    speed_12h_ago = df.groupby("storm_id")["speed_24h"].shift(2)  # t-24 to t-12
    accel_12h = (df["speed_24h"] - speed_12h_ago) / 12.0
    
    speed_24h_ago = df.groupby("storm_id")["speed_24h"].shift(4)  # t-48 to t-24
    accel_24h = (df["speed_24h"] - speed_24h_ago) / 24.0
    
    return accel_12h.fillna(0).values, accel_24h.fillna(0).values

def compute_heading_change(df: pd.DataFrame) -> np.ndarray:
    """
    Change in storm direction (deg/12h).
    For 24h forecast: recurvature detection critical.
    """
    heading_prev = np.arctan2(df["dx_24h"], df["dy_24h"])
    heading_12h_ago = df.groupby("storm_id")["heading_sin_24h"].shift(2)
    
    # Simplified: change in heading sine (proxy for direction shift)
    heading_change = df["heading_sin_24h"] - heading_12h_ago
    return heading_change.fillna(0).values

def compute_sst_gradient(df: pd.DataFrame) -> np.ndarray:
    """Spatial SST change (°C/100km)."""
    sst_grad = df.groupby("storm_id")["sst_c"].diff()
    dx = df.groupby("storm_id")["dx_24h"].transform(lambda x: x.fillna(0))
    dy = df.groupby("storm_id")["dy_24h"].transform(lambda x: x.fillna(0))
    dist = np.hypot(dx, dy)
...
def compute_persistence_tendency(df: pd.DataFrame) -> np.ndarray:
    """
    How stable is the motion? (std of last 48h speeds).
    Low variance → more persistent → persistence works better.
    High variance → steering changes → ML advantage.
    """
    speed_std = df.groupby("storm_id")["speed_24h"].rolling(window=8, min_periods=4).std()
    return speed_std.fillna(0).values

def build(args=None):
    p = argparse.ArgumentParser()
    p.add_argument("--source", default="outputs/model_ready_2000_2024_v2.parquet")
    p.add_argument("--out", default="outputs/track_forecast/features/track_features_2000_2024_24h.parquet")
    ...
    df = df[df["has_next24"]].copy()
    df = df.sort_values(["storm_id", "iso_time"]).reset_index(drop=True)
    
    # === Motion features (12h, 24h, 48h lookback) ===
    ...
    dx_48h, dy_48h, speed_48h, hsin_48h, hcos_48h = compute_motion_features(df, lookback=8)
    
    # Also 6h for short-term
    dx_6h, dy_6h, speed_6h, hsin_6h, hcos_6h = compute_motion_features(df, lookback=1)
    
    # Temporarily assign for downstream calcs
    df["dx_24h"] = dx_24h
    df["dy_24h"] = dy_24h
    df["speed_24h"] = speed_24h
    df["heading_sin_24h"] = hsin_24h
    
    # === Targets (t -> t+24h) ===
    dx_next, dy_next = [], []
    for lt, lo, ltn, lon in zip(df["lat"], df["lon"], df["lat_next24"], df["lon_next24"]):
    ...
    wind_chg_6h, wind_chg_12h, wind_chg_24h, wind_chg_48h = compute_intensity_change(df)
    motion_accel_12h, motion_accel_24h = compute_motion_acceleration(df)
    heading_change = compute_heading_change(df)
    sst_gradient = compute_sst_gradient(df)
    dist_to_land = compute_distance_to_land(df)
    motion_persistence = compute_persistence_tendency(df)
    
    # === Interactions ===
    intensity_x_speed = df["wmo_wind"].values * speed_24h
    ...
    # Environmental favorability (SST > 26.5°C for intensification)
    sst_favorable = (df["sst_c"].values > 26.5).astype(int)
    ...
    feat["dx_48h"] = dx_48h
    feat["dy_48h"] = dy_48h
    feat["speed_48h"] = speed_48h
    feat["heading_sin_48h"] = hsin_48h
    feat["heading_cos_48h"] = hcos_48h
    ...
    feat["wind_chg_48h"] = wind_chg_48h
    feat["motion_accel_12h"] = motion_accel_12h
    feat["motion_accel_24h"] = motion_accel_24h
    feat["heading_change"] = heading_change
    feat["sst_gradient"] = sst_gradient
    feat["dist_to_land_km"] = dist_to_land
    feat["motion_persistence"] = motion_persistence
    ...
    feat["sst_favorable"] = sst_favorable
    ...
    feat["lat_next24"] = df["lat_next24"].values
    feat["lon_next24"] = df["lon_next24"].values
    
    # Drop first 8 points per storm (need 48h lookback)
    feat = feat.dropna(subset=["dx_6h", "dy_6h", "dx_12h", "dy_12h", 
                                "dx_24h", "dy_24h", "dx_48h", "dy_48h"]).reset_index(drop=True)
